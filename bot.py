...
FORCE_GROUP = os.getenv("FORCE_GROUP")  # contoh: @groupusername
...
# ================= SAFE FUNCTIONS =================
async def safe_copy_to_storage(message):
    while True:
        try:
            return await message.copy(STORAGE_CHANNEL)
        except FloodWait as e:
            await asyncio.sleep(e.value)

async def safe_copy_to_user(client, user_id, msg_id):
    while True:
        try:
...
async def handle_media(client, message):
    ...
    try:
        forwarded = await safe_copy_to_storage(message)
...
async def send_page(client, user_id, page):
    ...
    for msg_id in file_ids[start:end]:
        await safe_copy_to_user(client, user_id, msg_id)
    ...
    if page > 1:
        row.append(InlineKeyboardButton("⬅", callback_data=f"page_{page-1}"))

    for p in range(1, total_pages + 1):
        if abs(p - page) <= 2:
            if p == page:
                row.append(InlineKeyboardButton(f"•{p}•", callback_data="current"))
            else:
                row.append(InlineKeyboardButton(str(p), callback_data=f"page_{p}"))
