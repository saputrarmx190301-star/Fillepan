...
active_workers = 3  # jumlah worker kirim file (bisa ubah 2-5)
...
async def safe_send_video(user_id, file_id):
    while True:
        try:
            await app.send_video(user_id, file_id)
...
async def worker():
    while True:
        user_id, file_id = await send_queue.get()
        await safe_send_video(user_id, file_id)
...
async def getcode(client, message):
    ...
    files = await db.fetch("SELECT size FROM files WHERE user_id=$1", user_id)
    if not files:
...
async def send_pack(message, code, page):
    ...
    for f in files[start:end]:
        await send_queue.put((user_id, f["file_id"]))
