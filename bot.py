...
FORCE_GROUP = os.environ.get("FORCE_GROUP")  # contoh: @groupkamu
...
async def start(client, message):
    ...
    if len(args) > 1:
        ...
        await message.reply("✅ Selesai!", reply_markup=end_btn)
        return
    ...
    await message.reply(
        "👋 Kirim beberapa file lalu tekan /create untuk membuat link.",
...
async def create_link(client, message):
    ...
    await message.reply("🗑 File chat dihapus.")
...
async def broadcast(client, message):
    ...
    async def send_broadcast(client, msg):
        ...
        for user in users:
            try:
                await msg.copy(user[0])
                sent += 1
            except:
                pass

        await msg.reply(f"✅ Broadcast terkirim ke {sent} user.")
