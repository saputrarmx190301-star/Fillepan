...
def join_button():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔔 Join Group", url=CHANNEL_LINK)]
    ])
...
def format_progress(total, uploaded):
    bar_len = 20
    filled_len = int(bar_len * uploaded / total)
    bar = "█" * filled_len + "░" * (bar_len - filled_len)
    return f"📦 Upload Progress: {uploaded}/{total}\n[{bar}]"
...
async def cmd_upload(client, message):
    ...
    await message.reply("📤 Kirim file/video sekarang. Progress animasi akan muncul di atas.")
...
async def cmd_create(client, message):
    ...
    if user_id not in user_sessions or not user_sessions[user_id]["files"]:
        return await message.reply("❌ Tidak ada file untuk dibuat pack.")
    ...
    await message.reply(f"🎉 Pack Created!\n\n🔗 {link}\n\n🔑 `{code}`", disable_web_page_preview=True, reply_markup=join_button())

    # Hapus progress message
    if session := user_sessions[user_id].get("progress"):
